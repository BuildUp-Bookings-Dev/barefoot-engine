...
class Property_Sync_Service
{
    ...
    private function sync_single_property_id(string $property_id, int $post_id): array|WP_Error
    {
        ...
        foreach ($properties as $property) {
            ...
            $candidate_id = isset($property['property_id']) && is_string($property['property_id'])
                ? trim($property['property_id'])
                : '';
        ...
        $result = $this->update_property_post($post_id, $existing, $matched, $started_at);
        if (is_wp_error($result)) {
            return $result;
        }
        ...
        if (isset($parsed['field_keys']) && is_array($parsed['field_keys'])) {
            $state['field_keys'] = $this->normalize_field_keys($parsed['field_keys']);
        }
